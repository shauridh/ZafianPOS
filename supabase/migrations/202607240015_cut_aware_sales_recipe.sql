alter table public.product_components
  add column if not exists cut_code text
  check (cut_code is null or cut_code in ('Dada','Sayap','Paha atas','Paha bawah'));

update public.product_components component
set cut_code = case
  when lower(item.name) like '%dada%' then 'Dada'
  when lower(item.name) like '%sayap%' then 'Sayap'
  when lower(item.name) like '%paha atas%' then 'Paha atas'
  when lower(item.name) like '%paha bawah%' then 'Paha bawah'
end
from public.inventory_items item
where item.id = component.inventory_item_id
  and component.is_cut_choice
  and component.cut_code is null;
  ...
  loop
    ...
    if nullif(v_item->>'product_id','') is not null then
      ...
      select exists(
        ...
        where product_id=v_product.id and is_cut_choice
      ) into v_has_cut_choice;

      if v_has_cut_choice and nullif(trim(v_item->>'chicken_cut'),'') is null then
        raise exception 'CUT_REQUIRED';
      end if;
      if v_has_cut_choice and not exists(
        select 1
        ...
        where component.product_id=v_product.id
          and component.is_cut_choice
          and lower(trim(coalesce(component.cut_code, inventory.name)))=lower(trim(v_item->>'chicken_cut'))
      ) then
        raise exception 'CUT_NOT_CONFIGURED:%', v_item->>'chicken_cut';
      end if;
      ...
      for v_component in
        ...
        where component.product_id=v_product.id
          and (
            not component.is_cut_choice
            or lower(trim(coalesce(component.cut_code, inventory.name)))=lower(trim(v_item->>'chicken_cut'))
          )
